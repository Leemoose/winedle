...
/* Ten blocks a row is honest but unreadable: it wraps in every messaging app,
 * and six rows of it is a wall. Each row is compressed to five blocks holding
 * the same proportions - red for exact, amber for close, blank for neither - so
 * the shape of closing in survives at a width that fits a phone. */
function barFor(tiles) {
  const hit = tiles.filter(t => t.state === 'hit').length;
  const near = tiles.filter(t => t.state === 'near').length;
  const total = tiles.length;

  let red = Math.round((hit / total) * BAR_WIDTH);
  let amber = Math.round((near / total) * BAR_WIDTH);

  /* Rounding can overshoot; trim the softer signal first, and never round a
   * genuine exact match away to nothing. */
  while (red + amber > BAR_WIDTH) {
    if (amber > 0) amber--;
    else red--;
  }
  if (red === 0 && hit > 0) {
    red = 1;
    if (red + amber > BAR_WIDTH) amber--;
  }

  return EMOJI.hit.repeat(red) + EMOJI.near.repeat(amber) +
         EMOJI.miss.repeat(BAR_WIDTH - red - amber);
}
...
function aromaLine(tiles) {
  ...
  if (t.kin && t.kin.length) {
    parts.push(t.kin.map(k => k.term.toLowerCase() + ' \u2192 ' + k.via.toLowerCase()).join(', '));
  }
...
function gridBars() {
  return state.guesses
    .map(n => barFor(compare(WINES.find(w => w.name === n), ANSWER)))
    .join('\n');
}

function triesPhrase() {
  if (state.status !== 'won') return 'I did not get this one';
  const n = state.guesses.length;
  return 'I got it in ' + n + (n === 1 ? ' try' : ' tries');
}
...
function challengeText() {
  return 'Winedle challenge \u2014 ' + triesPhrase() + '. Can you do better?\n\n' +
    gridBars() + '\n\nYour turn:\n' + SHARE_URL + '?w=' + encodeName(ANSWER.name);
}
...
/* Called once, at the moment the game ends - deliberately not from showEnd,
 * which re-runs on every render of a finished puzzle and so counted the same
 * win again on every reload and every new tab. */
function recordFinish(won) {
  recordOutcome(won);
  recordResult(won, state.guesses.length, DAY, !IS_ARCHIVE && !IS_PRACTICE && !IS_CHALLENGE);
  requestDurableStorage();
}

function showEnd() {
  const won = state.status === 'won';
  const stats = readJSON(KEY_STATS, blankStats());
  ...
  challenge.addEventListener('click', () => {
    offerCopy(challenge, challengeText(), 'Challenge a friend');
  });
...
function submitGuess(wine) {
  ...
  /* Before render, so the end panel shows stats that include this game. */
  if (state.status !== 'playing') recordFinish(state.status === 'won');
